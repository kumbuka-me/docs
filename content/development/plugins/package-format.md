...
`README.md` and `plugin.yaml` are required. `plugin.wasm` is required when the manifest contains an executable `renderer-extension`, `macro`, `code-highlighter`, `widget`, `content-change`, `exporter`, or `admin-action`. Declarative-only packages do not need WASM.
