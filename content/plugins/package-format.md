...
`README.md` and `plugin.yaml` are required. `plugin.wasm` is required when the manifest contains an executable `renderer-extension`, `macro`, `code-highlighter`, or `widget`. Declarative-only packages do not need WASM.
