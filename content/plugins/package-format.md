...
`README.md` and `plugin.yaml` are required. `plugin.wasm` is required when the manifest contains an executable `renderer-extension`, `macro`, or `code-highlighter`. Declarative-only packages do not need WASM.
