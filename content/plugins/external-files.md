...
The plugin is disabled by default. Its repository connections and presentation defaults are plugin-owned settings under **Administration → Plugin settings → External Files**; Kumbuka core provides only the generic typed-settings, resource, and outbound-HTTP capabilities.
...
5. For private repositories, add a dedicated read-only access token restricted to that repository.
6. For an internal provider, add the exact RFC1918 or IPv6 ULA addresses the provider hostname is allowed to resolve to.
...
- external content is not persisted in shared rendered-page artifacts.
